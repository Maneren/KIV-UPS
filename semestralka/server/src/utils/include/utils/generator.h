...
template <typename _Alloc> class __promise_base_alloc {
  ...
  static _Alloc &
  __get_allocator(void *__frame, std::size_t __frameSize) noexcept {
...
public:
  ...
  static void *operator new(
      std::size_t __frameSize, std::allocator_arg_t, _Alloc __alloc, _Args &...
  ) {
    ...
    ::new (static_cast<void *>(
        std::addressof(__get_allocator(__frame, __frameSize))
...
template <typename _Ref> struct __generator_promise_base {
  ...
  yield_value(std::ranges::elements_of<_Rng, _Allocator> &&__x) {
    return [](
               allocator_arg_t, _Allocator, auto &&__rng
           ) -> generator<_Ref, std::remove_cvref_t<_Ref>, _Allocator> {
      for (auto &&e : __rng)
...
public:
  using promise_type = __generator_promise<
      generator<_Ref, _Value>,
      __byte_allocator,
...
public:
  ...
  private:
    ...
    explicit iterator(
        __promise_base *__promise, std::coroutine_handle<> __coro
    ) noexcept
